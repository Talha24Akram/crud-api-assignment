...
My submission for FlyRank's Backend Track, Week 2 (Assignment A1), implementing a small CRUD API for a
to-do list, built with FastAPI.

## What this is

Five endpoints, full CRUD on a list of tasks: create one, read them, update them, delete them. Since there is
...
| GET    | `/tasks`      | List tasks (`?done=` and `?search=` filter them) | 200 | None                     |
...
| GET    | `/stats`      | `{"total", "done", "open"}` counts         | 200     | None                           |
| POST   | `/reset`      | Puts the 3 seed tasks back                 | 200     | None                           |
...
I did because `tasks[task_id]` is O(1), while my version does a linear scan through a list every time
(`find_task()`). A dict makes sense here since ids are already unique ints; I'd only need a list
if task order had to be independent of id. It's also just less code since there is no seed/reset helper or
...
1. **Blank titles get through.** `POST /tasks` with `{"title": ""}` still returns `201` and
   ...
   string, not that it's non-empty since there is no validator on it. Mine explicitly strips and
...
3. **Empty `PUT` body just silently succeeds.** Send `{}` to `PUT /tasks/1` and it comes back
   `200 OK` with the task completely unchanged, without returning any error at all. I never told
   the prompt to reject an empty update, so the AI never had a reason to think about that case.
...
- I didn't say what should happen if a `PUT` has neither field set, so it defaulted to "just
  succeed and change nothing", which isn't unreasonable, but not what the checkpoint expects either.
...
missing title, and always return 400 with a plain `{"error": "<message>"}` body for both, avoiding
...
Create a few tasks, restart the server, and they're gone, returning you back to the original 3. That's
